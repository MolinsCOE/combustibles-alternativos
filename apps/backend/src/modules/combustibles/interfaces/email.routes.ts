...
function buildHtml(body: EnviarCorreoBody): string {
  const { proveedorNom, materialNom, destino, semana, transportistaNom, viajes, confirmUrl } = body;
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto">
  <div style="background:#003E39;padding:20px 24px;border-radius:8px 8px 0 0">
    <h1 style="color:#fff;margin:0;font-size:1.2rem">Molins — Combustibles Alternativos</h1>
  </div>
  <div style="border:1px solid #e5e7eb;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      ...
      <tr>
        <th style="padding:8px;text-align:left;border:1px solid #e5e7eb">Destino</th>
        <td style="padding:8px;border:1px solid #e5e7eb">${destino}</td>
      </tr>` : ""}
      <tr>
    ...
    <p style="font-size:0.8rem;color:#9ca3af">Molins Cement — Dept. Compras</p>
  </div>
</div>`.trim();
}
...
// ---------------------------------------------------------------------------
// POST /api/combustibles/enviar-correo  (proveedor)
// ---------------------------------------------------------------------------
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      await sendMail({
        to: recipients.join(", "),
        bcc: body.bcc && body.bcc.length > 0 ? body.bcc.join(", ") : undefined,
        subject,
        html: buildHtml(body),
        text: buildText(body),
      });
...
// ---------------------------------------------------------------------------
// Schema y builders para el correo de transportista
// ---------------------------------------------------------------------------

const enviarCorreoTransportistaBodySchema = z.object({
  to: z.union([z.string().email(), z.array(z.string().email()).min(1)]),
  bcc: z.array(z.string().email()).optional(),
  transportistaNom: z.string().min(1),
  proveedorNom: z.string().min(1),
...
type EnviarCorreoTransportistaBody = z.infer<typeof enviarCorreoTransportistaBodySchema>;

function buildHtmlTransportista(body: EnviarCorreoTransportistaBody): string {
  const { transportistaNom, proveedorNom, materialNom, destino, semana, viajes, confirmUrl } = body;
  const total = viajes.l + viajes.m + viajes.x + viajes.j + viajes.v + viajes.s + viajes.d;
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto">
  ...
  <div style="border:1px solid #e5e7eb;border-top:none;padding:24px;border-radius:0 0 8px 8px">
    <p>Estimado/a <strong>${transportistaNom}</strong>,</p>
    <p>Os informamos de la planificación de transporte para la semana <strong>${semana}</strong>:</p>
    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      <tr style="background:#f3f4f6">
        <th style="padding:8px;text-align:left;border:1px solid #e5e7eb">Material</th>
        <td style="padding:8px;border:1px solid #e5e7eb">${materialNom}</td>
      ...
      <tr>
        <th style="padding:8px;text-align:left;border:1px solid #e5e7eb">Proveedor</th>
        <td style="padding:8px;border:1px solid #e5e7eb">${proveedorNom}</td>
      </tr>
    </table>
    <table style="width:100%;border-collapse:collapse;margin:16px 0;text-align:center">
      <tr style="background:#003E39;color:#fff">
    ...
    <div style="margin:24px 0;text-align:center">
      <a href="${confirmUrl}" style="background:#003E39;color:#fff;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:600;display:inline-block">
        Confirmar / Rechazar disponibilidad de transporte
      </a>
    </div>
    <p style="font-size:0.85rem;color:#6b7280">También puedes añadir comentarios desde el mismo enlace.</p>
    <hr style="border:none;border-top:1px solid #e5e7eb;margin:24px 0">
...
function buildTextTransportista(body: EnviarCorreoTransportistaBody): string {
  const { transportistaNom, proveedorNom, materialNom, destino, semana, viajes, confirmUrl } = body;
  const total = viajes.l + viajes.m + viajes.x + viajes.j + viajes.v + viajes.s + viajes.d;
  return (
    `Estimado/a ${transportistaNom},\n\n` +
    `Os informamos de la planificación de transporte para la semana ${semana}:\n\n` +
    `Material: ${materialNom}\n` +
    (destino ? `Destino: ${destino}\n` : "") +
    `Proveedor: ${proveedorNom}\n` +
    `Viajes: L${viajes.l} M${viajes.m} X${viajes.x} J${viajes.j} V${viajes.v} S${viajes.s} D${viajes.d}\n` +
    `Total semana: ${total} viajes\n\n` +
    `Para confirmar o rechazar la disponibilidad de transporte accede al siguiente enlace:\n` +
    `${confirmUrl}\n\n` +
    `Saludos,\nDept. Compras — Cementos Molins, S.A.`
  );
}
...
// ---------------------------------------------------------------------------
// POST /api/combustibles/enviar-correo-transportista  (transportista)
// ---------------------------------------------------------------------------

router.post(
  "/api/combustibles/enviar-correo-transportista",
  async (req: Request, res: Response): Promise<void> => {
    const parsed = enviarCorreoTransportistaBodySchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({
        code: "VALIDATION_ERROR",
        message: "Datos de entrada incorrectos",
    ...
    const body = parsed.data;
    const subject = `Validación de transporte — ${body.materialNom} — ${body.semana}`;
    const recipients = Array.isArray(body.to) ? body.to : [body.to];
    ...
    try {
      await sendMail({
        ...
        bcc: body.bcc && body.bcc.length > 0 ? body.bcc.join(", ") : undefined,
        subject,
        html: buildHtmlTransportista(body),
        text: buildTextTransportista(body),
      });
      res.status(200).json({ ok: true });
    } catch (err) {
      const message = err instanceof Error ? err.message : "Error desconocido";
